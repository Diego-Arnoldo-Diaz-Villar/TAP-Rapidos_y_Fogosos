...
# Obtener nombre del jugador desde los argumentos
if len(sys.argv) > 1:
    nombre_jugador = sys.argv[1]
else:
    nombre_jugador = "Jugador"

def enviar_puntuacion(nombre_jugador, puntuacion, host= '192.168.1.115' , puerto=5050):
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as cliente:
            cliente.connect((host,puerto))
            mensaje = f"{nombre_jugador}:{puntuacion}"
            cliente.sendall(mensaje.encode())

            # Recibir y mostrar los top scores
            respuesta = cliente.recv(1024).decode()
            print("[TOP RECORDS]")
            print(respuesta)
    except Exception as e:
        print(f"[ERROR LAN] No se pudo enviar la puntuación: {e}")


def iniciar_juego(nombre_jugador="Jugador"):
    pygame.init()
    ...
    while corriendo:
        ...
        if len(grupo_vehiculos) < 2:
            añadir_vehiculo = all(vehiculo.rect.top >= vehiculo.rect.height * 1.5 for vehiculo in grupo_vehiculos)
            if añadir_vehiculo:
        ...
        if pygame.sprite.spritecollide(jugador, grupo_vehiculos, True):
            ...
            enviar_puntuacion(nombre_jugador, puntuacion)
...
if __name__ == "__main__":
    nombre = sys.argv[1] if len(sys.argv) > 1 else "Jugador"
